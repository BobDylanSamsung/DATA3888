...
server <- function(input, output, session) {
  ...
  output$lambda_summary <- renderText({
    paste0(
  ...
  output$n_selected_genes <- renderText({
    ng <- nrow(gene_coefs)
  ...
  output$model_km_plot <- renderPlot({
  ...
  observeEvent(input$predict, {
    ...
    # Optional: render a prediction plot if available!
    # output$pred_plot <- renderPlot({ ... })
  })
