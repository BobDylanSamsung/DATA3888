...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      fileInput("gene_csv", "Upload CSV of genes (optional)", accept = c(".csv")),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$predict, {
    # 1. Ensure a CSV has been uploaded before proceeding
    req(input$gene_csv)
    
    # 2. Read the CSV as raw data (no header, so everything is in order)
    df <- read.csv(input$gene_csv$datapath, header = FALSE, stringsAsFactors = FALSE)
  
    # 3. The first row contains patientID header info — remove that row
    #    (df[1,2] would be patientID if you want to keep/display it)
    df <- df[-1, , drop = FALSE]
    
    # 4. Extract gene names and values from the columns
    ...
    # 5. Reformat just the genes your model expects, in the correct order
    newdata <- as.data.frame(t(values[selected_genes]))
    colnames(newdata) <- selected_genes
  
    # 6. Proceed to prediction exactly as before
