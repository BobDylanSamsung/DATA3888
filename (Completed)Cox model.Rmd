...
surv_test  <- surv_obj[test_idx ]

# training model
library(glmnet)
cox_model <- glmnet(x = x_train, y = surv_train, family  = "cox", alpha   = 1, lambda  = best_lambda)
coef_final <- coef(cox_model)

# Calculate the risk score
risk_train <- as.numeric(predict(cox_model, newx = x_train, type = "link"))
risk_test  <- as.numeric(predict(cox_model, newx = x_test , type = "link"))

# evaluation
library(survival)
c_train <- concordance(surv_train ~ risk_train, reverse = TRUE)$concordance
c_test  <- concordance(surv_test  ~ risk_test ,  reverse = TRUE)$concordance
cat(sprintf("C‑index (train): %.3f\n", c_train))
cat(sprintf("C‑index (test) : %.3f\n", c_test ))
```
On the test set, for any pair of patients, the model has approximately 78.9% probability of correctly determining which patient experienced the event (such as death) earlier.
...
After training the model, a total of 19 genes were selected.
