...
# GEO Data Analysis with Error Handling
...
library(pheatmap)
library(knitr)
library(BiocManager)  # In case we need to install any missing packages
```

# Utility Functions

```{r UTILITY_FUNCTIONS, warning=FALSE, message=FALSE}
...
get_geo_data <- function(gse_id, destdir = tempdir(), retries = 3) {
  message(paste("Attempting to download", gse_id))
  ...
  for (i in 1:retries) {
    tryCatch({
      ...
      message(paste("Successfully downloaded", gse_id))
      return(gse)
      
    }, error = function(e) {
      message(paste("Attempt", i, "failed for", gse_id, ":", e$message))
      
      if (i < retries) {
        message(paste("Retrying in 5 seconds..."))
        Sys.sleep(5)
      } else {
...
tryCatch({
  # Approach 1: Standard download with custom directory
  GSE78229_gse <- get_geo_data("GSE78229", destdir = download_dir)
  GSE78229 <- extract_gse_data(GSE78229_gse)
  
}, error = function(e) {
  message(paste("Error with approach 1:", e$message))
  
  # Approach 2: Try with temporary directory
  tryCatch({
    GSE78229_gse <- get_geo_data("GSE78229", destdir = tempdir())
    GSE78229 <- extract_gse_data(GSE78229_gse)
    
  }, error = function(e2) {
    message(paste("Error with approach 2:", e2$message))
    
    # Approach 3: Try without GSEMatrix
    tryCatch({
      GSE78229_gse <- getGEO("GSE78229", GSEMatrix = FALSE)
      # For GSEMatrix = FALSE, we need different extraction
      stop("GSEMatrix = FALSE requires different processing. Please check your connection and try again.")
      
    }, error = function(e3) {
      stop(paste("All approaches failed. Please check your internet connection and try again.\nLast error:", e3$message))
    })
  })
})
...
# Load other datasets
tryCatch({
  GSE28735_gse <- get_geo_data("GSE28735", destdir = download_dir)
  GSE28735 <- extract_gse_data(GSE28735_gse)
  
  GSE62452_gse <- get_geo_data("GSE62452", destdir = download_dir)
  GSE62452 <- extract_gse_data(GSE62452_gse)
...
loaded_datasets <- c()
if (exists("GSE78229")) loaded_datasets <- c(loaded_datasets, "GSE78229")
if (exists("GSE28735")) loaded_datasets <- c(loaded_datasets, "GSE28735")
if (exists("GSE62452")) loaded_datasets <- c(loaded_datasets, "GSE62452")

message(paste("Successfully loaded datasets:", paste(loaded_datasets, collapse = ", ")))
```
...
```{r CHECK_NETWORK, warning=FALSE, message=FALSE}
# Check current timeout settings
current_timeout <- getOption("timeout")
message(paste("Current timeout setting:", current_timeout, "seconds"))

# Increase timeout if needed (useful for slow connections)
options(timeout = 300)  # 5 minutes

# Check proxy settings if you're behind a firewall
# Sys.setenv(http_proxy = "http://your.proxy.server:port")
# Sys.setenv(https_proxy = "https://your.proxy.server:port")
```

# Data Exploration (once loaded)

```{r EXPLORE_DATA, warning=FALSE, message=FALSE}
# This section will only run if data was successfully loaded
if (exists("GSE78229")) {
  message("GSE78229 data structure:")
  message(paste("- Expression matrix dimensions:", paste(dim(GSE78229$eMat), collapse = " x ")))
  message(paste("- Number of features:", nrow(GSE78229$featureData)))
  message(paste("- Number of samples:", nrow(GSE78229$phenoData)))
  
  # Preview the data
  message("\nFirst few gene IDs:")
  print(head(rownames(GSE78229$eMat)))
  
  message("\nFirst few sample IDs:")
  print(head(colnames(GSE78229$eMat)))
}
```
...
for (tp in time_points) {
  cat("\n====", tp, "Months Survival Prediction Evaluation ====\n")
  
  # 1. Construct the actual label and explicitly set it to factor to ensure there are two levels
  test_pheno$actual_status <- ifelse(
    test_pheno$is_dead == 1 & test_pheno$months_survived <= tp, 
    "High", 
    "Low"
  )
  test_pheno$actual_status <- factor(
    test_pheno$actual_status, 
    levels = c("Low", "High")
  )
  
  # 2. Screen out evaluable samples
  evaluable <- which(
    test_pheno$is_dead == 1 | test_pheno$months_survived >= tp
  )
  if (length(evaluable) < 10) {
    warning("Too few samples for evaluation at ", tp, " months")
    next
  }
  eval_pheno <- test_pheno[evaluable, ]
  
  # 3. For the prediction label, the same factor levels should also be locked
  eval_pheno$predicted_risk <- factor(
    eval_pheno$predicted_risk, 
    levels = c("Low", "High")
  )
  
  # 4. Generate a 2×2 confusion matrix
  cm <- table(
    Predicted = eval_pheno$predicted_risk, 
    Actual    = eval_pheno$actual_status
  )
  print(cm)
  
  # 5. Calculate a series of indicators directly using caret
  cm_caret <- confusionMatrix(
    cm, 
    positive = "High"
  )
  cat("Accuracy:   ", round(cm_caret$overall["Accuracy"] * 100, 2), "%\n")
  cat("Sensitivity:", round(cm_caret$byClass["Sensitivity"] * 100, 2), "%\n")
  cat("Specificity:", round(cm_caret$byClass["Specificity"] * 100, 2), "%\n")
  cat("Precision:  ", round(cm_caret$byClass["Precision"] * 100, 2), "%\n")
  
  # 6.Calculate ROC/AUC (return probability)
  probs <- as.numeric(predict(
    final_model, 
    newx = x_test[evaluable, ], 
    type = "response"
  ))
  roc_obj <- roc(
    eval_pheno$actual_status, 
    probs, 
    levels = c("Low", "High"), 
    direction = "<"
  )
